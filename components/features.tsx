...
const features = [
  {
    icon: Search,
    title: "Monitor",
    description: "Track mentions across Google, Facebook, LinkedIn, YouTube, Reddit",
  },
  {
    icon: BarChart3,
    title: "Analyze",
    description: "AI sentiment analysis in Sinhala, Tamil & English with predictive insights",
  },
  {
    icon: MessageSquare,
    title: "Engage",
  ...
  },
  {
    icon: TrendingUp,
    title: "Improve",
    description: "Trend prediction and issue extraction to prevent reputation crises",
  },
]
          ...
          {features.map((feature, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Icon className="w-6 h-6" style={{ color: "var(--royal-blue)" }} />
                </div>
                <h3 className="text-xl font-bold mb-2" style={{ color: "var(--pure-black)" }}>
                  {feature.title}
                </h3>
                <p style={{ color: "var(--charcoal)" }}>{feature.description}</p>
              </div>
            )
          })}
        </div>
